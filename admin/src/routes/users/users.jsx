    ...
    return (
        <div className="max-w-6xl p-8 mx-auto">
            <header className="mb-12 text-center">
                ...
                <div className="p-8 text-white transition-shadow duration-300 shadow-lg bg-gradient-to-r from-red-500 to-red-600 rounded-xl hover:shadow-xl">
            ...
            <main className="flex flex-col gap-6 md:grid md:grid-cols-2 lg:grid-cols-3">
                {users.length > 0 ? (
                    users.map((user, index) => (
                        <div 
                            key={user._id} 
                            className={`pb-6 ${index !== users.length - 1 ? "border-b border-gray-200 md:border-b-0" : ""}`}
                        >
                            <UserDetails
                                user={user}
                            />
