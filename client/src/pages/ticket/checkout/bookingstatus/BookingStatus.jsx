...
const BookingStatus = ({ id, routeTo, selectedSeats, totalCost }) => {
  ...
  const makePayment = async () => {
    try {
      ...
      const stripe = await loadStripe("pk_test_51QhB8bKL0PphUpNSPVajkCGGZQInLLXE8OiEvMqf82Rg740jFXP2Oxa1lxJbxedDPmH2cQOPVKri91dBff2cAkjV002RtaIZVH");
      if (!stripe) throw new Error("Failed to load Stripe.js");

      const amountInCents = Math.round(Number(totalCost));
      if (isNaN(amountInCents) || amountInCents <= 0) throw new Error("Invalid totalCost");

      console.log("Sending request to backend...");
      const response = await fetch("http://localhost:8000/api/buses/create-checkout-session", {
        method: "POST",
      ...
      if (response.status !== 200) throw new Error(data.message || "Payment failed");
          ...
          <div className="space-y-2 w-full">
            ...
            <div className="space-y-0.5 w-full">
              <div className="flex items-center justify-between w-full gap-x-5">
                <p className="text-sm font-normal text-neutral-400">From <span className="text-xs">{data.busCitiesAndTimes[0].cityName}</span></p>
                <p className="text-sm font-normal text-neutral-400">To <span className="text-xs">{matchingCity.cityName}</span></p>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center w-full gap-x-3">
              {selectedSeats.length > 0 ? (
                selectedSeats.map(seat => (
                  <div key={seat} className="flex items-center justify-center text-base font-semibold rounded-lg w-9 h-9 bg-neutral-200/80 text-neutral-700">
                    {data.seats.find(s => s._id === seat)?.seatNumber || "N/A"}
                  </div>
                ))
              ) : (
          ...
          <div className="space-y-2 w-full">
            ...
            <div className="flex items-center justify-between gap-x-4">
              <div className="flex gap-y-0.5 flex-col">
                ...
                <span className="text-xs font-normal text-neutral-500">(Including all taxes)</span>
              </div>
              <p className="text-base font-semibold text-neutral-600">Rs {totalCost}</p>
            </div>
          </div>
        </div>
      </div>
      <div className="w-full px-1.5">
        <button onClick={makePayment} className="w-full text-sm font-normal bg-primary hover:bg-primary/90 text-neutral-50 py-2.5 flex items-center justify-center uppercase rounded-lg transition">
